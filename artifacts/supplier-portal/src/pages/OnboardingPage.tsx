...
import { CheckCircle, Building01, Tag01 } from "@untitledui/icons";
...
export default function OnboardingPage() {
  ...
  // Roots only — sub-categories implied by serving the parent.
  ...
  const handleSubmit = () => {
    if (selectedCategoryIds.size === 0) {
      toast({ variant: "destructive", title: "Pick at least one category", description: "We need to know what you can supply to route RFQs your way." });
      return;
    }
    completeOnboarding.mutate(
      {
        data: {
          ...
          categories_served: [...selectedCategoryIds],
        },
      },
      {
        onSuccess: () => {
      ...
      <main className="flex-1 flex items-start justify-center px-6 py-10">
        <div className="w-full max-w-xl space-y-6">
          ...
          {step === 2 && (
            <div className="bg-white rounded-2xl border border-[rgb(228,231,236)] shadow-[0_1px_3px_rgba(0,0,0,0.06)] p-6">
              <div className="flex items-center gap-3 mb-4">
                <Tag01 className="h-6 w-6 text-[rgb(255,109,67)]" />
                <div>
                  <h1 className="text-xl font-semibold text-[rgb(16,24,40)]">Categories you serve</h1>
                  <p className="text-sm text-[rgb(102,112,133)]">
                    We'll route matching RFQs to you. Pick all that apply.
                  </p>
                </div>
              </div>
                  ...
                  {rootCategories.map((cat) => {
                    ...
                    return (
                      <label
                        ...
                        data-testid={`onboarding-cat-${cat.slug}`}
                      >
                        <input
                          type="checkbox"
                          checked={checked}
              ...
              <p className="mt-3 text-xs text-[rgb(152,162,179)]">
                Selected: <span className="font-medium text-[rgb(52,64,84)]">{selectedCategoryIds.size}</span>
              </p>
          ...
          {step === 3 && (
            <div className="bg-white rounded-2xl border border-[rgb(228,231,236)] shadow-[0_1px_3px_rgba(0,0,0,0.06)] p-8 text-center">
              ...
              <p className="mt-2 text-sm text-[rgb(102,112,133)]">
                Add your first offer to start receiving auto-quotes for matching RFQs.
              </p>
              <div className="mt-6 flex justify-center gap-2">
                <Button onClick={() => setLocation("/offers/new")} variant="outline" data-testid="onboarding-go-create-offer">
                  Add an offer
                </Button>
