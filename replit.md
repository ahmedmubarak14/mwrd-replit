...
1. **Landing page** (`/`): `curl -s http://localhost:80/ | grep -ciE 'grovia|MWRD|mwrd-enhanced|http://localhost|replit\.dev'` should print `0`. View-source must show "Procure for your business" hero copy and lowercase `mwrd` brand directly in HTML. Title tag must read `mwrd — AI-powered procurement…`.
2. **Auth round-trip**: all three demo logins return a token via `POST /api/auth/login` with the credentials from the table above.
3. **Artifact paths**: `/`, `/client/`, `/supplier/`, `/backoffice/`, `/api/healthz` all return 200.
4. **Cross-links**: client/supplier login pages link logo back to `/`. Landing "Sign in" injection points to the correct portal.
